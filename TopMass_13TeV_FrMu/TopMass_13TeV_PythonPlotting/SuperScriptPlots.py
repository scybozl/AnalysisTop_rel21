...
import VariableList
from SendToCluster import *
from MassDictionaries import *
from DefineSamples import *
from ListSystematics import *

# values taken from https://twiki.cern.ch/twiki/bin/viewauth/AtlasProtected/TopDerivationMC16aList
...
def PrepareInputTrees(Chain, Channel, NormFactorList, OutputFile, Cuts, DSID, TreeName):
    ...
    if Channel == "lepjets":
        fT_interm.SetBranchStatus("tma_klf_mtop_param", 1)
        fT_interm.SetBranchStatus("tma_klf_orig_mw",   1)
        fT_interm.SetBranchStatus("tma_klf_orig_rbq",  1)
    else:
...
def FillHistograms(DSID, InputFolder, OutputFile, DataMCFlag, Tree, Weight, Channel, Flag, Cuts):
    ...
    if "MC16e" in DataMCFlag:
        Lumi = Lumi_2018 
        
    HelpDataMCFlag = DataMCFlag

    if "data" in DataMCFlag or "Data" in DataMCFlag:
        if "2018" in OutputFile:
            HelpDataMCFlag = "MC16e"
        elif "2017" in OutputFile:
    ...
    VarList    = VariableList.ReturnVariableListSimple(DataMCFlag, VariableFlag, True)
    ...
    if "data" in DSID or "grp" in InputFolder:
        ...
        if "data16" in DSID or "grp16" in InputFolder:
            Flag = "2016"
        if "data18" in DSID or "grp18" in InputFolder:
            Flag = "2018"
    ...
    if not "Data" in DataMCFlag and not "data" in DataMCFlag:  
    ...
    fF_out  = TFile(OutputFile, "RECREATE")
    FullWeight = "weight_mc*weight_pileup*weight_leptonSF*weight_jvt*weight_bTagSF_MV2c10_70*"+str(NormFactor)
    ...
    OutputFileTree = OutputFile.replace("/Hist", "/Tree")
...
def MergeHistos(InputFolder, List, Name, InputFlags):
    ...
    OutputFile     = OutputFolder+"/Merge_Hist_"+Name+"_"+Label+".root"
    ...
    if not os.path.exists(OutputFile):
        ...
        for InputFlag in InputFlags:
            ...
            for entry in List[0]:
                #print entry
                File     = InputFolder+"/Hist_"+entry+"_"+InputFlag+".root"
        ...
        #print len(MergeString),"   ",counter,"   ",doMerge
...
def MergeAllHistos(InputFolder):
    ...
    InputFlags = [["MC16a"], ["MC16d"], ["MC16a", "MC16d", "MC16e"], ["data"]]
...
def FillHistosAndTrees(Channel, CutList, MainInputFolder, OutputFolder, OutputFolderFlag, Type, RunType, MergeHistos):

    # now get all names of systematic trees in the fullsim samples                                                                                                                                       
    SystTreeList = ReturnTreeList()


    InputFolderList  = glob.glob(MainInputFolder+"/*Files_*") 
    ...
    for InputFolder in InputFolderList:
        ...
        if not "SYS" in RunType:
            TreeList = ["nominal"]
                
        for TreeName in TreeList:
            
            NewOutputFolder         = OutputFolder+"/Output_"+Channel+CutFlag+OutputFolderFlag+"/Out_"+InputFolder.split("Files_")[1]
            MainOutputFolderChannel = NewOutputFolder+"/Unmerged_"+TreeName+"/"
            SubmissionFolder        = NewOutputFolder+"/SubmissionFolder/"+RunType+"_"+TreeName+"_FS/"
            if "_AF" in NewOutputFolder:
                SubmissionFolder = NewOutputFolder+"/SubmissionFolder/"+RunType+"_"+TreeName+"_AF/"

            os.system("mkdir -p "+MainOutputFolderChannel)
            os.system("mkdir -p "+SubmissionFolder)

            SendToCluster(InputFolder, MainOutputFolderChannel, SubmissionFolder, Channel, TreeName, WeightName, Type, Cuts)

            if MergeHistos:
                SuperScriptPlots.MergeAllHistos(MainOutputFolderChannel)


def DoParametrisation(Channel, ConfigFile):

    histo_name_list = []

    if Channel == "dilepton":
        histo_name_list.append("h_mlb")
    else:
        histo_name_list.append("h_mtop_param")
        histo_name_list.append("h_mw_orig")
        histo_name_list.append("h_rbq_orig")
    

    for histo_name in histo_name_list:

        # separate fit to find starting values for combined fit
        os.system("RunSepFitPar "+histo_name+" "+ConfigFile)   
        # combined fit
        os.system("RunCombFitPar "+histo_name+" "+ConfigFile)

        # need config file in MainInputFolder, containing only relative paths
        # need to hand over MainInputFolder
        # need to hand over OutputFolder
